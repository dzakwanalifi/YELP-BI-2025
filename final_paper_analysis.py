...
try:
    ...
    df_prices['commodity_name'] = df_prices['commodity_name'].str.strip() # CLEAN WHITESPACE
    
    # Filter for Jakarta & West Java cities only (Study Area)
    # UPDATED: Matching actual CSV values (without 'Kota' prefix)
    study_cities = ['DKI Jakarta', 'Jakarta Pusat', 'Bandung', 'Bogor', 'Bekasi', 
                    'Depok', 'Sukabumi', 'Tasikmalaya', 'Cirebon',
                    'Kab. Bogor', 'Kab. Sukabumi', 'Kab. Bandung', 'Kab. Bekasi', 'Kab. Cirebon', 'Kab. Tasikmalaya']
...
try:
    ...
    # UPDATED: Direct mapping since names are cleaner now
    loc_map = {
    ...
    if len(reg_df) > 2:
        ...
        print(f"   ! Not enough data points for regression. Merged DF size: {len(reg_df)}")
        print(f"     Price Locs: {avg_price_loc['mapped_loc'].unique()}")
        print(f"     Dist Locs: {avg_dist_loc['search_location'].unique()}")
...
try:
    ...
    loc_weather = 'Bandung' # UPDATED: Correct name
...
try:
    ...
    print(f"ERROR IN PRODUCTION MAP: {e}")
